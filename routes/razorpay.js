...
const {validatePlanId, getPlanInfo, generateSubscriptionRef} = require('../utils')
    ...
    try {
        ...
        if (!plan_id) {
            return res.status(400).json({
                ...
                available_plans: Object.values(config.planConfig).map(plan => plan.id),
            });
        }
...
router.post("/verify-payment", (req, res) => {
    try {
        const { razorpay_payment_id, razorpay_subscription_id, razorpay_signature, customer_email } = req.body;
...
router.post("/cancel-subscription", async (req, res) => {
    try {
        ...
        const result = await razorpay.subscriptions.cancel(subscription_id, cancel_at_cycle_end);
